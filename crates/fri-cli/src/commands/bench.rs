...
pub fn benchmark(cmd: BenchCommand) {
    ...
    for path in paths {
        ...
        let compression_rate =
            (uncompressed_size as f32 - (result.len()) as f32) / uncompressed_size as f32 * 100.;
        let frif_bpp = result.len() as f32 / (height * width) as f32 * 8.;
        let png_bpp = png_size as f32 / (height * width) as f32 * 8.;
        let png_compression_rate =
            (uncompressed_size as f32 - png_size as f32) / uncompressed_size as f32 * 100.;
    ...
    let avg_compression_rate =
        compression_rates.iter().sum::<f32>() / compression_rates.len() as f32;
    let avg_png_compression_rate =
        compression_rates_png.iter().sum::<f32>() / compression_rates_png.len() as f32;
